...
var Meyda = {
  ...
  extract: function extract(feature, signal, previousSignal) {
    ...
    if (!signal) throw this._errors.invalidInput;else if (_typeof(signal) != 'object') throw this._errors.invalidInput;else if (!feature) throw this._errors.featureUndef;else if (!utilities.isPowerOfTwo(signal.length)) throw this._errors.notPow2; // ~~~ UNNECESSARY CODE ~~~
...
var prepareSignalWithSpectrum = function prepareSignalWithSpectrum(signal, windowingFunction, bufferSize) {
  ...
  preparedSignal.complexSpectrum = (0, _fftjs.fft)(preparedSignal.windowedSignal); // ~~~ ORIGINAL CODE ~~~
